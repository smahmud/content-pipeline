...
class TestEnhancedAdapterProtocol:
    ...
    def test_adapter_implements_enhanced_protocol(self):
        """Test that WhisperAdapter implements all enhanced protocol methods."""
    ...
    def test_validate_requirements_model_load_failure(self, mock_load_model):
        ...
        assert isinstance(errors, list)
        assert len(errors) > 0
        assert any("Failed to load Whisper model" in error for error in errors)
    ...
    def test_validate_requirements_invalid_model(self, mock_load_model, mock_available_models):
        ...
        adapter = WhisperAdapter(model_name="invalid_model")
        errors = adapter.validate_requirements()
        
        assert isinstance(errors, list)
        assert len(errors) > 0
        assert any("Model 'invalid_model' not available" in error for error in errors)
    ...
    def test_get_engine_info_returns_tuple(self):
        ...
        adapter = WhisperAdapter(model_name="large")
        ...
        assert engine_info[0] == "whisper"
        assert engine_info[1] == "large"

    def test_adapter_protocol_compliance(self):
        """Test that WhisperAdapter is recognized as a TranscriberAdapter."""
        adapter = WhisperAdapter(model_name="base")
