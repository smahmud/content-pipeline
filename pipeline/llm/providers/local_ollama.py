...
Local deployment provider for Ollama models (llama2, mistral, codellama, etc.).
...
File naming follows pattern: local_{service}.py
Provider ID: local-ollama

This module was migrated from pipeline.enrichment.agents.local_ollama_agent
as part of the infrastructure refactoring. All hardcoded configuration has been
removed and replaced with configuration objects.
"""
...
class LocalOllamaProvider(BaseLLMProvider):
    ...
    This provider integrates with a local Ollama service to provide LLM capabilities
    ...
    Configuration is loaded from OllamaConfig, which supports:
    ...
    def __init__(self, config: OllamaConfig):
        ...
        Args:
            config: Configuration for the Local Ollama provider
        """
        self.config = config
    ...
    def _make_request(self, endpoint: str, data: Optional[Dict] = None) -> Dict:
        ...
        Raises:
            ProviderNotAvailableError: If cannot connect to Ollama
        ...
        try:
            ...
            raise ProviderError(f"Ollama API request failed: {str(e)}")
